...
Visualisation is often done using bar charts:
...
The standard deviation of a Poisson distribution conveniently equals the square root of lambda, $SD = \sqrt{\lambda}$. When working with case rates, $SD = \sqrt{\lambda / n_i}$, where n~i~ is the size of i^th^ time interval (the area of opportunity).
...
Instead, to find the probability of a value being less than or equal to a given threshold, we use `pnorm()`, which returns the area under the curve up to that value. For example, `pnorm(3500, mean = avg_w, sd = std_w) = ` ``r  pnorm(3500, mean = avg_w, sd = std_w)`` indicates that approximately 51.6% of birth weights are expected to be below 3500 grams.
