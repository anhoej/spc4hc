...
For example, if the probability that a single data point falls outside the three-sigma limits is 0.3% (see below), then the probability that all 20 data points in a control chart fall within the limits is
...
Most importantly, it follows that we can never be absolutely certain that signals always represent special causes, or that the absence of signals always represents common causes. Probability -- and statistics in general -- is therefore not a tool for certainty. Rather, it is a tool for making informed judgements under uncertainty by quantifying how likely different explanations are in light of the available data.

>[...] prediction within limits means that we can state, at least approximately, the probability that the observed phonomenon will fall within the given limits.
>
>@shewhart1931, p. 6

In this context, *past experience* may derive from empirical observations, theoretical considerations, or both, concerning the distribution and variability of data collected over time. The remainder of this chapter describes how such knowledge can be obtained and used in practice.
...
* Poisson (event counts/rates)
* Binomial (case counts/proportions)
