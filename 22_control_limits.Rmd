...
Generally, data come in two flavours: count data and measurement data. **Counts** are positive integers that represent counts of events or cases, for example surgical complications, healthy babies, or patient falls. **Measurements** are data that are measured on continuous scales and may have decimals, for example blood pressure, height and weight, or waiting times.
...
* deaths (numeric): number of patients who died within 30-day after all-cause (hospital or community acquired) infection
* patients (numeric): number of patients with all-cause infection

We think of infections as events and patients as cases. The difference may seem insignificant, but consider infections as events that can occur more than once to a patient, while a patient is either infected or not infected and either dead or alive.

### C chart

The C charts (C for counts) is the simplest of all control charts and the easiest to calculate. The process standard deviation is simply estimated as the square root of the process mean.
...
with(bact, {
  ...
  # plot the chart
  spc(month, ha_infections, cl, lcl, ucl,
      ylab = 'Infections',
      xlab = 'Month')
})
```

The average number of cases is 22.7 per month and all data points are within the control limits ranging from 8.4 to 36.9. 
...
In cases like this where the denominator -- the area of opportunity -- only varies little between subgroups, the U charts adds little compared to the C chart. However, when one needs to compare events from organisational units of different size or over time periods where the area of opportunity varies significantly, it may be useful to compare the rates rather than the raw counts.

### P chart

P charts are for proportions (or percentages). Figure \@ref(fig:limits-p shows the monthly proportion of patients with bacteremia who died within 30 days. In contrast to event rates, where the numerator (event counts) and the denominator (area of opportunity) are different things, proportions always shows the number of cases in relation to the total number of cases and non-cases. This means that proportions always are dimensionless. Percentages are simply proportions multiplied by 100.

```{r limits-p, fig.cap='P chart of monthly 30-day mortality rates'}
with(bact, {
  y   <- deaths / patients
  cl  <- sum(deaths) / sum(patients)
  s   <- sqrt((cl * (1 - cl) / patients))
  lcl <- cl - 3 * s                          # lower control limit
  ucl <- cl + 3 * s                          # upper control limit
...
On average the mortality is 21%. All data points are within the control limits and there are no unusually long runs or unusually few crossings. The the process appears stable and predictable.
