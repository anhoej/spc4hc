...
Cutting dates is such a common task that we may want to make our own function:
...
How old is R (at the time of writing)?

```{r}
today <- as.Date('2025-04-24')
r_dob <- as.Date('2000-02-29')

# calculate age in days (using parentheses to print result)
(age_in_days <- today - r_dob)
```

When subtracting (or adding) dates we get back a difftime object. Usually we want to convert this to an integer:
...
Notice that this calculation may be off by up to one day depending on the number and position of leap years. However, this is far better than relying on age expressed in whole years. The error is on the third decimal. So, when presenting data, rounding to one or two decimal places seems reasonable:

```{r}
round(age_in_years, 1)
```
...
* Use lower case letters (a-z) and numbers (0-9), e.g. "myvariable", "myfile.R".
