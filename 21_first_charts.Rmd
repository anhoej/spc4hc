...
From Part 1 of this book we have a good grasp of what SPC is and how SPC charts work. In this chapter we will start constructing SPC charts using functions from base R. In later chapters we will include functions from `ggplot2`and `qicharts2`.
...
Consider these data, which show systolic blood pressure measurements (mm Hg) for a patient taken in the morning over 26 consecutive days [@mohammed2008].
...
To guide the runs analysis we will add the centre line, which in run charts is usually the median of the data points (Figure \@ref(fig:first-chart-run2)):
...
In the next chapter we will calculate lines and limits for the SPC charts that are most commonly used in healthcare, "The Magnificent Seven".
