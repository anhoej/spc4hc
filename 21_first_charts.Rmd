...
cl  <- rep(median(systolic), length(systolic))
...
# Create systolic-coordinates for the control limits
lcl <- cl - 3 * s
ucl <- cl + 3 * s
```

Before we plot data, we need to expand the systolic-axis limits to make room not only for the data points but also the control limits (Figure \@ref(fig:first-chart-ctrl1)):

```{r first-chart-ctrl1, fig.cap='Standardised control chart'}
# Plot data while expanding the systolic-axis to make room for all data and lines
