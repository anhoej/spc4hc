...
The Bernoulli CUSUM chart (or B chart) is a specialised SPC chart designed to detect shifts in case proportions over time. The B chart takes a logical vector (TRUE/FALSE) and plots a trace value, which is a cumulative sum of deviations from a target value. The trace value$s_i$ is calculated at each observation:

$$s_i=s_{i-1}+ increment\ based\ on\ current\ value$$

When the current value (x~i~) is TRUE, the trace increases, and when it is FALSE, the trace decreases.

The size of the increment is based on the target value and the size of the shift we want to detect.
...
If the trace remains near zero, it means that the process is operating close to target as in Figure \@ref(fig:rare-fig6), which is constructed using the bchart() function from qicharts2.

```{r rare-fig6, fig.cap='B chart of newborns with asphyxia'}
bchart(births$asphyxia, target = 0.007, or = 2)
# bchart(births$asphyxia, target = 500)
# bchart(births$asphyxia, target = 0.001, or = 2, limit = 3.5)
# bchart(births$asphyxia, target = 0.002)
# bchart(births$asphyxia, target = mean(births$asphyxia), or = 2)
```
