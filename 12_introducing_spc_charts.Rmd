...
Figure \@ref(fig:spc-fig1) demonstrates a standardized Shewhart control chart, created using random numbers from a normal distribution with a mean of 0 and a standard deviation of 1. The x-axis represents time or sequence order, while the y-axis displays the indicator values. Each dot, connected by a line, represents a data point sampled sequentially. 

The chart features a Centre Line (CL), which represents the mean of the data values, and a Lower Control Limit (LCL) and an Upper Control Limit (UCL), defining the range of common cause variation. The lower and upper control limits are often referred to as 3-sigma limits.
...
* I and MR chart: individual measurement, subgroup size = 1.
* X-bar and S chart: multiple measurements, subgroup size > 1.
